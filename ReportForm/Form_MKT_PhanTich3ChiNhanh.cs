...
{
    public partial class Form_MKT_PhanTich3ChiNhanh : Form
    {
        public Form_MKT_PhanTich3ChiNhanh()
        {
            InitializeComponent();
        }
        SqlConnection con;
        private void Form_MKT_PhanTich3ChiNhanh_Load(object sender, EventArgs e)
        {
        ...
        private async void btntimkiemngay_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //SqlCommand cmd = new SqlCommand("MKT_PhanTich3ChiNhanh_2023", con);
                ...
                //reportphantich3chinhanh.ProcessingMode = ProcessingMode.Local;
                //reportphantich3chinhanh.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_MKT_Phantich3chinhanh.rdlc";
                //if (ds.Tables[0].Rows.Count > 0)
                //{
                //    ReportDataSource rds = new ReportDataSource();
                //    rds.Name = "DataSetPhantich3chinhanh";
                //    rds.Value = ds.Tables[0];
                //    reportphantich3chinhanh.LocalReport.DataSources.Clear();
                //    reportphantich3chinhanh.LocalReport.DataSources.Add(rds);
                ...
                //    reportphantich3chinhanh.LocalReport.SetParameters(reportParameters);
                //    reportphantich3chinhanh.RefreshReport();
                //}
                try
                {
                    dtMerged.Clear();
                    ...
                    {
                        // URL của Node.js API
                        string url = $"http://synthesisreport.tmhsg.com/get_marketing_phantich3chinhanh_2024?startDate={startDateTime:yyyy-MM-ddTHH:mm:ss}&endDate={endDateTime:yyyy-MM-ddTHH:mm:ss}";
                ...
                reportphantich3chinhanh.ProcessingMode = ProcessingMode.Local;
                reportphantich3chinhanh.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_MKT_Phantich3chinhanh.rdlc";
                //reportphantich3chinhanh.LocalReport.ReportPath = "Rp_MKT_Phantich3chinhanh.rdlc";
                reportphantich3chinhanh.LocalReport.DataSources.Clear();

                // Thêm dữ liệu từ DataTable hợp nhất (PostgreSQL và SQL Server)
                ReportDataSource rdsMerged = new ReportDataSource("DataSetPhantich3chinhanh", dtMerged);
                reportphantich3chinhanh.LocalReport.DataSources.Add(rdsMerged);
