...
{
    public partial class Form_CP_ChiPhiNhapVPP : Form
    {
        public Form_CP_ChiPhiNhapVPP()
        {
            InitializeComponent();
        }
        SqlConnection con;
        SqlConnection con2;
        private void Form_CP_ChiPhiNhapVPPcs_Load(object sender, EventArgs e)
        {
        ...
        private void btntimkiemngay_Click(object sender, EventArgs e)
        {
            if (cbbcongty.SelectedItem == "Đa khoa")
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("CP_ChiPhiNhapVPP_2023", con);
                    ...
                    dap.Fill(ds);
                    reportChiphivpp.ProcessingMode = ProcessingMode.Local;
                    reportChiphivpp.LocalReport.ReportPath =@"\\server-one\SynthesisReports$\Rp_CP_Chiphinhapvpp.rdlc";
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ReportDataSource rds = new ReportDataSource();
                        rds.Name = "DataSetChiphinhanvpp";
                        rds.Value = ds.Tables[0];
                        reportChiphivpp.LocalReport.DataSources.Clear();
                        reportChiphivpp.LocalReport.DataSources.Add(rds);
                        ...
                        reportChiphivpp.LocalReport.SetParameters(reportParameters);
                        reportChiphivpp.RefreshReport();
                    }
                    con.Close();
                }
                catch (Exception ex)
            ...
            }else if(cbbcongty.SelectedItem == "Bệnh viện")
            {
                try
                {

                    SqlCommand cmd = new SqlCommand("CP_ChiPhiNhapVPP_2023", con2);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@TuNgay", TuNgay.Value.ToString("yyyy/MM/dd"));
                    cmd.Parameters.AddWithValue("@DenNgay", DenNgay.Value.ToString("yyyy/MM/dd"));
                    cmd.Parameters.AddWithValue("@NhaCungCap_ID", cbbnhacungcap.SelectedValue);
                    ...
                    dap.Fill(ds);
                    reportChiphivpp.ProcessingMode = ProcessingMode.Local;
                    reportChiphivpp.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_CP_Chiphinhapvpp_bv.rdlc";
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ReportDataSource rds = new ReportDataSource();
                        rds.Name = "DataSetChiphinhapvppbv";
                        rds.Value = ds.Tables[0];
                        reportChiphivpp.LocalReport.DataSources.Clear();
                        reportChiphivpp.LocalReport.DataSources.Add(rds);
                        ReportParameterCollection reportParameters = new ReportParameterCollection();
            ...
            else
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("CP_ChiPhiNhapVPP2CN_2023", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@TuNgay", TuNgay.Value.ToString("yyyy/MM/dd"));
                    cmd.Parameters.AddWithValue("@DenNgay", DenNgay.Value.ToString("yyyy/MM/dd"));
                    cmd.Parameters.AddWithValue("@NhaCungCap_ID", cbbnhacungcap.SelectedValue);
                    ...
                    dap.Fill(ds);
                    reportChiphivpp.ProcessingMode = ProcessingMode.Local;
                    reportChiphivpp.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_CP_Chiphinhapvpp2cn.rdlc";
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ReportDataSource rds = new ReportDataSource();
                        rds.Name = "DataSetChiphinhapvpp2cn";
                        rds.Value = ds.Tables[0];
                        reportChiphivpp.LocalReport.DataSources.Clear();
                        reportChiphivpp.LocalReport.DataSources.Add(rds);
                        ReportParameterCollection reportParameters = new ReportParameterCollection();
