...
{
    public partial class Form_DT_Doanhthuksktoandien : Form
    {
        public Form_DT_Doanhthuksktoandien()
        {
            InitializeComponent();
        }
        SqlConnection con;
        private void Form_DT_Doanhthuksktoandien_Load(object sender, EventArgs e)
        {
        ...
        private async void btntimkiemngay_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //SqlCommand cmd = new SqlCommand("DT_DoanhThuKSKToanDien_2023", con);
                ...
                //dap.Fill(ds);
                //reportDoanhthuksktoandien.ProcessingMode = ProcessingMode.Local;
                //reportDoanhthuksktoandien.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_DT_Doanhthuksktoandien.rdlc";
                //if (ds.Tables[0].Rows.Count > 0)
                //{
                //    ReportDataSource rds = new ReportDataSource();
                //    rds.Name = "DataSetDoanhthuksktoandien";
                //    rds.Value = ds.Tables[0];
                //    reportDoanhthuksktoandien.LocalReport.DataSources.Clear();
                //    reportDoanhthuksktoandien.LocalReport.DataSources.Add(rds);
                ...
                //    reportDoanhthuksktoandien.LocalReport.SetParameters(reportParameters);
                //    reportDoanhthuksktoandien.RefreshReport();
                //}
                //con.Close();
                try
                {
                    ...
                    {
                        // URL của Node.js API
                        string url = $"http://synthesisreport.tmhsg.com/get_doanhthu_goitoandien_2024?startDate={startDateTime:yyyy-MM-ddTHH:mm:ss}&endDate={endDateTime:yyyy-MM-ddTHH:mm:ss}";
                ...
                reportDoanhthuksktoandien.ProcessingMode = ProcessingMode.Local;
                reportDoanhthuksktoandien.LocalReport.ReportPath = @"\\server-one\SynthesisReports$\Rp_DT_Doanhthuksktoandien.rdlc";
                //reportDoanhthuksktoandien.LocalReport.ReportPath = "Rp_DT_Doanhthuksktoandien.rdlc";
                reportDoanhthuksktoandien.LocalReport.DataSources.Clear();

                // Thêm dữ liệu từ DataTable hợp nhất (PostgreSQL và SQL Server)
                ReportDataSource rdsMerged = new ReportDataSource("DataSetDoanhthuksktoandien", dtMerged);
                reportDoanhthuksktoandien.LocalReport.DataSources.Add(rdsMerged);
